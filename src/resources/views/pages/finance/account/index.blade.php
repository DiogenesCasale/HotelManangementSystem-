...
@section('content')
    ...
    <h1 class="text-xl font-semibold mt-4 text-black dark:text-white">Contas</h1>
    ...
    <div class="flex items-center mt-2">
        ...
        <x-buttons.indigo class="text-sm w-32 ml-auto" id="create-account-button">Cadastrar</x-buttons.indigo>
    </div>

    <x-tables.table headerComponent="tables.finance.account.header" rowComponent="tables.finance.account.row" class="mt-4"
        id="table" />
    ...
    <x-modal.modal-sidebar-right id="filter-modal" class="hidden" title="Filtros"
        ...
        <form class="grid grid-cols-12 gap-2">
            ...
            <div class="col-span-12">
                ...
                <x-inputs.input name="value" type="text" placeholder="R$ 0,00" />
            </div>
    ...
    <x-modal.modal class="hidden w-2xl" id="account-modal" title="Cadastrar conta"
        description="Preencha os campos para cadastrar uma nova conta">
        <x-forms.account />
    </x-modal.modal>
...
@section('scripts')
    @vite('resources/js/pages/finances/accounts/index.js')
@endsection
